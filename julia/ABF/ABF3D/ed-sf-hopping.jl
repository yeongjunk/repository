...
include("./ed-sf-hopping-params.jl") # read parameters from configuration file
function abf3d_scan(p::Params)
    q_str = "q=".*string.(p.q)
    rng = MersenneTwister(p.seed)
    for (i, Li) in enumerate(p.L)
        ltc = Lattice3D(Li, Li, Li, 2)
        for (j, θj) in enumerate(p.θ)
            for (k, Vk) in enumerate(p.V)
                fn = "d3abf-sf-hopping-pn-L$(Li)-th$(round(θj, digits = 3))-V$(Vk)-R$(p.R[i]).csv" #File name
                ...
                # Add disorder & detangle & project
                    ...
                    D_phase = hop_dis(H, V = Vk, rng = rng)
