...
function fit_gipr(L::AbstractVector{Int64}, gipr::AbstractVector{Float64}, q::AbstractVector{Float64})
    @assert size(gipr, 1) == length(q)
    log_L = log(L)
    log_gipr = log(gipr)
    X = [ones(length(b)) log_L]
    Y = log_gipr
    v = X\Y
    return v
end

function mf_spectrum(q, τ_q)
    @assert length(q) == lenght(τ_q)
    α = diff(τ_q)./(q[2] - q[1])
