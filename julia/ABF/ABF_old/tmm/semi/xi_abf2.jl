...
"""
function config_read(config::Dict)
...
@doc """
Compute localisation length by scanning the parameters
"""
function ξ_scan(p::Parameters; logscale = false)
    E_all, W_all, θI_all, θII_all, r_all = params_expand(p)
    if logscale == true
        W_all = map(x -> 10^x, W_all)
    end
    return ξ_scan_core(E_all, W_all, θI_all, θII_all, r_all)
end

function ξ_scan_core(E_all::Array{Float64}, W_all::Array{Float64}, θI_all::Array{Float64}, θII_all::Array{Float64}, r_all::Array{Float64})
    ...
    @time Threads.@threads for i in 1:length(E_all)
        for j in 1:length(W_all), k in 1:length(θI_all), l in 1:length(θII_all)
            ξ_all[i, j, k, l] = ξ_abf2(E_all[i], W_all[j], θI_all[k], θII_all[l], r_all)
        end
    end
    return ξ_all
end
#
# @doc """
# (Multi-threaded)Compute localisation length by scanning the parameters. This is specially designed to quickly calculate diagonal
# """
# function ξ_scan_diag(p::Parameters)
#     E_all, W_all, θI_all, θII_all, r_all = params_expand(p)
#     return ξ_scan_core(E_all, W_all, θI_all, r_all)
# end


# @doc """
# This is specially designed to calculate diagonal maximum.
# """
# function ξ_scan_diag_core(E_all::Array{Float64}, W_all::Array{Float64}, θI_all::Array{Float64}, r_all::Array{Float64})
#     ξ_all = Array{Float64}(undef, length(E_all), length(W_all), length(θI_all))
#     #combining nested for loop int o one for loop to increase the possible limit of number of threads
#     @time Threads.@threads for i in 1:length(W_all)
#         for j in 1:length(E_all), for k in 1:length(θI_all)
#             ξ_all[i, j, k] = ξ_abf2(E_all[i], W_all[j], θI_all[k], θI_all[k], r_all)
#         end
#     end
#     return ξ_all
# end
