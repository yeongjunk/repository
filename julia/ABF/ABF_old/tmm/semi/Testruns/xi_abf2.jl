...
function ξ_scan_core(E_all::Array{Float64}, W_all::Array{Float64}, θI_all::Array{Float64}, θII_all::Array{Float64}, r_all::Array{Float64})
    ...
    @time Thread.@threads for k in 1:length(θII_all)*length(θI_all)
        x = div(k-1,length(θI_all))+1
        y = mod1(k,length(θII_all))
...
@doc """
(Distributed)Compute localisation length by scanning the parameters, currently, θI_num and θII_num shold be equal.
"""
