...
function blockdiagonal(A_block, N)
    ...
    rowidx = Array{Int64}(repeat(1:m*N, inner = m))
    colidx = Array{Int64}(repeat(1:m:m*N, inner = m*m) + repeat(0:m-1, outer = m*N))
...
function projection(H_FD)
    N = size(H_FD, 1)÷2
    H_SF = spzeros(ComplexF64, N, N)
    rows = rowvals(H_FD)
    vals = nonzeros(H_FD)

    for j = 2:2:2N
       for i in nzrange(H_FD, j)
          if iseven(rows[i])
              H_SF[rows[i]÷2,j÷2] = vals[i]
          end
      end
    end
