...
function scan_ταf(f::Function, params, E_c, E_del, ltc::Lattice, L::Int; c=1., seed::Int = 1234, isherm::Bool = true, l::Vector{Int} = [1, 2],  q::Vector{Float64} = [2.], R::Int = 10, nev::Int= 10)
    @assert length(l) >= 2
    nt = Threads.nthreads()
    ...
    return E_mean, gipr_mean, μqlnμ_mean 
end
