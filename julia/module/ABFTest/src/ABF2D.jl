...
function redef1_obc(ltc::Lattice2D; vartype = Float64)
    num_sites = ltc.M*ltc.N*ltc.U
    I = Int64[]; J = Int64[]; V = vartype[]
    for m in 1:ltc.M, n in 1:ltc.N - 1
        push!(I, index(ltc, (m,n,1)))
        push!(J, index(ltc, (m,n,1)))
        push!(V, one(vartype))
...
function redef2_obc(ltc::Lattice2D; vartype = Float64)
    num_sites = ltc.M*ltc.N*ltc.U
    I = Int64[]; J = Int64[]; V = vartype[]
    for m in 1:ltc.M-1, n in 1:ltc.N
        push!(I, index(ltc, (m,n,1)))
        push!(J, index(ltc, (m,n,1)))
        push!(V, one(vartype))
...
function ham_fe(ltc::Lattice2D, Ea::F, Eb::F, θ::F) where F 
    U = U_fe(ltc, θ)
...
function ham_fe(ltc::Lattice2D, Ea::F, Eb::F; θ1::F = 0.25, θ2::F = 0.25, θ3::F = 0.25) where F 
    U1 = LUT(ltc,θ1)
    U2 = LUT(ltc,θ2)
    U3 = LUT(ltc,θ3)
    T1 = redef1_obc(ltc, vartype = F)
